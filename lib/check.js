...
function checkRangeDashes(lines, file) {
  const warnings = [];
  const rangeRe = /\b\d{1,4}(?::\d{1,3})?-\d{1,4}\b/g;
  ...
  lines.forEach((line, index) => {
    ...
    while ((match = rangeRe.exec(line)) !== null) {
      ...
      warnings.push(makeWarning(
        ...
        match.index + 1,
        'range-dash',
        `Numeric range "${match[0]}" uses a single hyphen.`,
        `Use keyboard-friendly double hyphen source syntax for ranges: "${match[0].replace('-', '--')}".`,
      ));
    }
  });
